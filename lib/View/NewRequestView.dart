...
class _NewRequestViewState extends State<NewRequestView> {
  ...
  void initState() {
    ...
    LocalDB.getLDB('userID').then((value) {
      ...
      _api.fetchRequestDetails(params);
    });
    super.initState();
  }
  @override
            ...
            body:SingleChildScrollView(
              child: Column(
                children: [
                  ...
                  Container(
                    height: height * 0.8,
                    child:ChangeNotifierProvider<NewRequestVM>(
                      create: (BuildContext context) => _api,
                      child: Consumer<NewRequestVM>(builder: (context, viewModel, _) {
                        switch (viewModel.getNewRequest.status) {
                          case Status.Loading:
                            return  const Center(child: CircularProgressIndicator());
                          case Status.Error:
                            return const Center(child: Center(child: Text("Something Issue"),))  ;
                          case Status.Completed:
                            var VM = viewModel.getNewRequest.data!;
                            return
                                      ...
                                      children: [
                                        ListView.builder(
                                          ...
                                          itemBuilder: (BuildContext context, int index) {
                                            return InkWell(
                                              onTap: (){
                                                NavigateController.pagePush(context, PatientDetailsView(screenType: 2, bookingType: bookingType, bookingID:  VM.lstPatientResponse![index].bookingId.toString(), data2: {},));
                                              },
                                              child: Padding(
